...
export default function SideNav({ links, isCollapsed, className, closeNav }: NavProps) {
  ...
  return (
    <div
      ...
      className={cn(
        "group mt-8 border-b bg-background py-2 transition-[max-height,padding] duration-500 data-[collapsed=true]:py-2 md:border-none",
        className,
      )}>
...
function NavLink({ title, icon, label, href, closeNav, subLink = false }: NavLinkProps) {
  ...
  return (
    <Link
      ...
      className={cn(
        buttonVariants({
          variant: checkActiveNav(href) ? "sideLink" : "ghost",
          size: "sm",
        }),
    ...
    <Collapsible defaultOpen={isChildActive}>
      <CollapsibleTrigger
        className={cn(
          ...
          "group h-12 w-full justify-start px-6",
        )}>
        <div className="mr-2">{icon}</div>
        {title}
        {label && (
