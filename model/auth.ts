...
import { apiCaller, publicApiCaller } from "@/lib/auth";
...
const useAuthStore = create<AuthState>()((set) => ({
  ...
  login: async (formData) => {
    try {
      const res = await publicApiCaller.post<LoginApiRes>("/api/auth/login-with-otp/", formData);
  ...
  logout: async () => {
    try {
      ...
      await apiCaller.post("api/auth/revoke-access-token/", null, {
        headers: {
